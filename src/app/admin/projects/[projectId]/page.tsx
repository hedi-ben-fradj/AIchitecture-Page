...
function ViewCard({ view, onDelete }: { view: View, onDelete: (viewId: string) => void }) {
    ...
    return (
        <>
            ...
            <Link href={view.href}>
                <Card className="bg-[#2a2a2a] border-neutral-700 text-white rounded-lg h-full cursor-pointer hover:border-yellow-500 transition-colors group relative">
        ...
        <div className="flex flex-col h-full bg-[#313131]">
            ...
            <div className="flex-1 p-8">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {views.map((view) => (
                        <ViewCard key={view.id} view={view} onDelete={deleteView} />
                    ))}
                     <Card 
                        onClick={() => setIsAddModalOpen(true)}
                        className="bg-[#2a2a2a] border-neutral-700 text-white flex flex-col items-center justify-center min-h-[200px] rounded-lg border-2 border-dashed border-neutral-600 hover:border-yellow-500 hover:text-yellow-500 cursor-pointer transition-colors"
                     >
